...
t = 0:1/Fs:T-1/Fs; % Create a time vector 't' representing the time instances at which the
% samples of the chirp signal will be evaluated. The time vector starts
% from 0 and increments at a rate of 1/Fs (sampling interval) up to the
% duration 'T' (in seconds), with a total of length(T*Fs) samples.
...
% Calculate the normalized angular frequency for the notch filter
wo = frequencies / (Fs/2);

% Calculate the bandwidth for the notch filter
bw = wo / Q;

% Design an IIR notch filter using the calculated parameters
...
% Play filtered signal (Warning: Loud)
