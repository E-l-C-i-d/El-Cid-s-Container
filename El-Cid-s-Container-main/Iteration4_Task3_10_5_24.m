...
function [b, a] = peq(Fs, frequencies, Q, gain)
    ...
    % Design peaking equalizer with bandwidth (BW) set to 1/50th of the center frequency
    BW = wo / 50;
    
    % Design peaking equalizer
    [b, a] = iirpeak(wo(1), BW(1), Q, gain); % Design for the first frequency
    % Apply peaking equalizer for the second frequency
    [b2, a2] = iirpeak(wo(2), BW(2), Q, gain); 
